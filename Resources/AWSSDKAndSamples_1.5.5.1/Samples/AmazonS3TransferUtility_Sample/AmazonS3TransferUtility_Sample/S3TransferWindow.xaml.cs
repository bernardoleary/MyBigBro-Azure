...
namespace AmazonS3TransferUtility_Sample
{
    ...
    public partial class S3TransferWindow : Window, INotifyPropertyChanged
    {
        ...
        private void loadConfiguration()
        {
            ...
            {
                MessageBox.Show(this, "AWSAccessKey is not set in the App.Config", "Error", MessageBoxButton.OK, MessageBoxImage.Error);
                this.Close();
                return;
            }
            ...
            {
                MessageBox.Show(this, "AWSSecretKey is not set in the App.Config", "Error", MessageBoxButton.OK, MessageBoxImage.Error);
                this.Close();
                return;
            }
        ...
        private void threadedUploadFile(object state)
        {
            try
            {
                ...
                TransferUtilityUploadRequest request = new TransferUtilityUploadRequest()
                    ...
                    .WithFilePath(this.UploadFile)
                    .WithTimeout(FIVE_MINUTES)
                    .WithSubscriber(this.uploadFileProgressCallback); // Pass in a callback so upload progress can be tracked.
